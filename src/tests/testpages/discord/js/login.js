...
document.getElementById("getDiscordURL").addConnection(function (text) {
    ...
    let discordhref = input.redirectUrl + "&redirect_uri=" + encodeURIComponent(redirect_url);
...
if (code != null & state != null) {
    document.getElementById("getjws").setAttribute("data-options", JSON.stringify({ "body": "{\"code\":\"" + code + "\", \"state\":\"" + state + "\", \"redirectUri\":\"" + redirect_url + "\", \"name\":\"Name\", \"surname\":\"surName\",\"adult\":1, \"school_id\":1}" }))
}

document.getElementById("getjws").addConnection(function (text) {
    const input = JSON.parse(text);
    localStorage.setItem("jws", input.jws)
    document.getElementById("token-out").setAttribute("value", `Bearer ${input.jws}`)
});
